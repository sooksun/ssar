...
import {
  addTeachingMediaFile,
  deleteTeachingMediaFile,
  setPrimaryTeachingMediaFile,
  updateTeachingMediaFile,
} from '@/app/actions/teaching-media';
...
export default function TeachingMediaFilesForm({
  teachingMediaId,
  files,
}: {
  teachingMediaId: string;
  files: FileItem[];
}) {
      ...
      (async () => {
        try {
          ...
          if (hasFiles) {
            ...
            const response = await fetch(`/api/teaching-media/${teachingMediaId}/files`, {
              method: 'POST',
              body: formData,
            });
            ...
            res = await addTeachingMediaFile(formData);
          }
  ...
  return (
    <div className="space-y-8">
      ...
      <form action={handleSubmit} className="space-y-4 rounded-lg border bg-card p-6">
        ...
        <input type="hidden" name="teachingMediaId" value={teachingMediaId} />
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <input
              name="fileName"
              placeholder={storageType === 'URL' ? 'ไม่กรอกจะใช้ชื่อไฟล์เดิม' : 'เช่น วิดีโอแนะนำบทที่ 1'}
          ...
          {!isLinkType && (
            <div className="space-y-2 md:col-span-2">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                <br />
                • ตั้งเป็นไฟล์หลัก (thumbnail ของสื่อการสอน) ได้เฉพาะรูปภาพ (JPG, JPEG, PNG) - ให้ user เป็นผู้กำหนดเอง
              </p>
            </div>
          )}
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <textarea
              name="description"
              rows={2}
              placeholder="เช่น ภาพการนำสื่อไปใช้ในชั้นเรียน"
          ...
          {files.length === 0 ? (
            ...
            files.map((f) => {
              ...
              const linkHref = isLinkType 
                ...
                : `/teaching-media/${teachingMediaId}/files/${f.id}/view`;
                          ...
                          onClick={() =>
                            startTransition(() => {
                              (async () => {
                                ...
                                fd.set('teachingMediaId', teachingMediaId);
                                fd.set('fileId', f.id);
                                ...
                                try {
                                  const res = await updateTeachingMediaFile(fd);
                                  if (!res.success) {
                                    await Swal.fire({
                                      icon: 'error',
                      ...
                      <Button
                        ...
                        onClick={() =>
                          startTransition(() => {
                            (async () => {
                              try {
                                const res = await setPrimaryTeachingMediaFile(teachingMediaId, f.id);
                                if (!res.success) {
                                  await Swal.fire({
                                    icon: 'error',
                      ...
                      <Button
                        ...
                        onClick={() =>
                          startTransition(() => {
                            (async () => {
                              try {
                                const res = await deleteTeachingMediaFile(teachingMediaId, f.id);
                                if (!res.success) {
                                  await Swal.fire({
                                    icon: 'error',
